            ...
            for br in big_rings:
                ...
                for edge in G.edges():
                    if G.get_edge_data(*edge)["bond_type"] != "SINGLE":
                        ...
                        cut_shared
                        and len(shared_edges.get(edge, [])) > min_shared_edges
                    ):
                        continue
                        ...
                        s_atom = (
                            G.node[s_idx]["atom"]
                            if G.node[s_idx]["atom"] == "C"
                            else "Y"
                        )
                        e_atom = (
                            G.node[e_idx]["atom"]
                            if G.node[e_idx]["atom"] == "C"
                            else "Y"
                        )
    ...
    def findRings(self):
        ...
        if len(smallest_rings) == 0:
            ...
            if len(smallest_rings[0]) > 10:
                ...
                return [list(smallest_rings[0])], []
        set_big = {
            i for i, ringa in enumerate(smallest_rings) if len(ringa) > 10
        }
        G = nx.Graph()
        ...
        for bond in self.rdmol.GetBonds():
            ...
            if bond.GetBondType() != Chem.rdchem.BondType.SINGLE or (
                begin_idx in atom_ring_idx
                and atom_ring_idx.get(end_idx, -1) == atom_ring_idx[begin_idx]
            ):
...
if __name__ == "__main__":
    ...
    all_atoms = set(range(cmpd.rdmol.GetNumAtoms()))
    for fr in chain(frags.frag_rings, frags.frag_chains):
        emol = Chem.rdchem.EditableMol(cmpd.rdmol)
        frag_atoms = set(fr)
